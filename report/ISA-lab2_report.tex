...
% table with the current situation
	...
	This implies that the maximum error is related to the weight of the column in which the compressor has been allocated: it's easy to understand that an approximated compressor allocated on the a \textit{left-most} bit creates an error higher than the one produced by a compressor put more on the right.
	...
	\paragraph{Setting the environment} First passage consists of connecting to the server and cleaning working directories locally and remotely. Then input sample vectors are generated and uploaded to the server with the specific scripts and all the common files needed for simulation. In this first step, model of the IIR filter and of a multiplier are run to produce correct results, usefull for the final analysis.
